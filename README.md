...
如果 agent 客户端在 07:17 本地时间**离线**，但电脑开机且用户会话可用，
**调度任务**会跑完整 daily。Windows 默认任务不保证在电脑关机、睡眠或用户
未登录时执行；需要更强覆盖时，应额外配置登录后补跑、唤醒运行或常开设备。
**心跳和调度互补，不冲突**。
...
agent 在后台注册一个每日任务，让机器和用户会话可用时 digest 自动生成。
**用户不敲命令**——只对 agent 说"每天 07:17 跑一次"。
...
- 默认注册为当前用户任务，通常不需要管理员权限
- 电脑需要开机，且当前用户会话可用；锁屏通常可以，关机/睡眠/未登录不保证
- 如果错过 07:17，可额外配置"登录后补跑"来生成当天日报
- 如果要未登录运行、唤醒电脑或使用最高权限，需要额外系统设置，可能触发 UAC/管理员权限

**调度 ≠ 实时**：

- 调度**只**覆盖"机器可用时 digest 自动生成"，不等同于电脑关机也能执行
- 评论 / 回复 / heartbeat 仍需要 agent 偶尔在线
- 用户电脑经常关机 → 使用登录后补跑、唤醒运行或常开设备，并让 agent 偶尔上线，才能完整覆盖
