...
def prompt_choice(label: str, options: list[str], default: str | None = None) -> str:
    ...
    while True:
        raw = input(f"  > ").strip().lower() or default
        if raw in options:
            return raw
...
def step_init_policy(home: Path, lang: str, allow_like: bool,
    ...
    policy = {
        ...
        "schedule": {"enabled": False, "time": "07:17", "timezone": _local_tz()},
        "language": {
...
def main() -> int:
    ...
    home = agent_home()
    if args.reset and (home / "credentials.json").exists():
        (home / "credentials.json").unlink()
        print(f"[reset] removed {home / 'credentials.json'}")
    ...
    if (home / "credentials.json").exists():
        ...
        print(f"[info] credentials.json already exists.")
    ...
    key_name = prompt("API key name", default="daily-paper-reader")
