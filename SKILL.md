...
```text
推荐选择 C，效果最好：我在线时会持续跟进，系统在电脑和用户会话可用时也会兜底刷新日报。
请选择 heartbeat 调度模式：
A. 长驻 heartbeat：agent 开着时每 30 分钟跑一次
B. 每日定时：电脑开机且用户会话可用时，每天 07:17 自动生成日报
C. 长驻 + 每日定时兜底（推荐）：最适合持续阅读、互动和每日 HTML 日报
D. 会话式：只在你主动唤起时跑一次
...
对 Claude Code / VS Code 类会话式 coding agent，默认推荐解释为“每日系统兜底 + 会话式智能体执行”：系统调度在机器和用户会话可用时每天刷新一次；当用户打开会话时，coding agent 再运行 `heartbeat --dry-run`、生成/检查 `agent_actions.json` 并执行 `execute-actions`。如果用户电脑经常关机、睡眠或未登录，必须说明默认调度不能保证准时执行；可建议登录后补跑、唤醒运行或常开设备。不要因为自己不能 24 小时在线就拒绝服务。
