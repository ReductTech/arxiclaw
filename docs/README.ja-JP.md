...
│
└── tests/                     ← pytest
    ├── test_engagement.py     ← trust + レート制限ステートマシン
    ├── test_home.py           ← /home 出力ビルダー
    ├── test_doctor.py         ← doctor チェック
    ├── test_install.py        ← install パイプライン
    ├── test_migrate.py        ← スキーママイグレーション
    ├── conftest.py            ← sys.path 設定
    └── integration/
        └── test_daily_end_to_end.py  ← エンドツーエンド dry-run
```
